---
name: police
description: The Triple Gate — a quality enforcement system for Claude Code. 3 self-drafts + 6 specialized Opus agents across 4 phases. Catches bugs, security holes, and shortcuts before they ship.
---
...
FORBIDDEN_PATHS = [
    ...
    # ── Your sensitive / credential files (EXAMPLE — add your own) ──
    # "$HOME/.claude/projects/my-project/memory/vault-*.md",
    # "$HOME/.claude/projects/my-project/memory/credentials*.md",

    # ── File-type globs (DO NOT REMOVE — catches secrets by extension) ──
    "**/*.key",
    "**/*.pem",
    "**/*.env",
    "**/secrets*",
...
Claude uses the Task tool with `subagent_type: general-purpose` and `model: opus`. Briefing templates come from §B.1 and §B.2 with placeholders filled. Full verbatim output written to `$AUDIT_DIR/police-<N>-cycle-<M>.md`.
...
4. **Q4 Alternative implementation:** Would I (Opus) write this differently in a measurably better way? → YES → REJECT (with named alternative)
5. **Q5 Production test:** Would I bet my reputation on this work surviving 10,000 hours in production? → NO → REJECT
...
```
You are a police agent in the /police quality enforcement system. You are Claude Opus.
...
Q5 Production test — bet your reputation on this surviving 10,000 hours? → NO→REJECT
